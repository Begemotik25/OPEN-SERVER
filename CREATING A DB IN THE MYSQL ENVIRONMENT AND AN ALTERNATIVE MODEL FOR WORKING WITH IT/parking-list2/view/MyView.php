...
class MyView extends ParkingListView {
    private function showParkings($parkings) {
        ...
        <form method='get' name='parking-form'>
            ...
            <?php if($this->checkRight('parking', 'create')): ?>
                <a href="?action=create-parking">Додати парковку</a>
            <?php endif; ?>
        </form>
        <?php
    }
    ...
    private function showParking(\Model\Parking $parking) {
        ...
        <div class="control">
            <?php if($this->checkRight('parking', 'edit')): ?>
                <a href="?action=edit-parking-form&parking=<?php echo $_GET['parking']; ?>">Редагувати парковку</a>
            <?php endif; ?>
            <?php if($this->checkRight('parking', 'delete')) :?>
                <a href="?action=delete-parking&parking=<?php echo $_GET['parking']; ?>">Видалити парковку</a>
            <?php endif; ?>
        </div>
        <?php
    }
        ...
        <section>
            <?php if ($_GET['parking']): ?> 
                <?php if($this->checkRight('car', 'create')):?>
                    <div class="control">
                        <a href="?action=create-car-form&parking=<?php echo $_GET['parking']; ?>">Додати авто</a>
                    </div>
                <?php endif; ?>
        ...
        <?php foreach ($cars as $key => $car): ?>
        	...
        	<tr class = "<?php echo $row_class; ?>">
                ...
                <?php if($this->checkRight('car','edit')):?>
                        <a href="?action=edit-car-form&parking=<?php echo $_GET['parking']; ?>&file=<?php echo $car->getId();?>">Редагувати</a>
                    <?php endif; ?>
                    <?php if($this->checkRight('car','delete')):?>
                        <a href="?action=delete-car&parking=<?php echo $_GET['parking']; ?>&file=<?php echo $car->getId();?>">Видалити</a>
                    <?php endif; ?>
                </td>
            </tr>
            <?php endif; ?>
    ...
    public function showParkingEditForm(\Model\Parking $parking) {
        ...
        <form name="edit-parking" method="post" action="?action=edit-parking&parking=<?php echo $_GET['parking']; ?>">
            <div>
    ...
    public function showCarCreateForm() {
        ...
        <body>
        <a href="?parking=<?php echo $_GET['parking']; ?>">На головну</a>
        <form name='create-car' method="post" action="?action=create-car&parking=<?php echo $_GET['parking']; ?>">
            <div>
                <label for='car_number'>Номер машини: </label>
            ...
            <table>
                ...
                <?php foreach($users as $user):?>
                    <?php if($user->getUserName() != $_SESSION['user'] && $user->getUserName() != "admin" && trim($user->getUserName()) != '' ): ?>
                        <tr> 
                            <td><a href="?action=edit-user-form&username=<?php echo $user->getUserName(); ?>">
